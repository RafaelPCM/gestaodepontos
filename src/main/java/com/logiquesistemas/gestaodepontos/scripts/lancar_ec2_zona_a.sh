...
if [ -z "$security_group_id" ]; then
    echo ">[ERRO] Security group gestaodepontos-dev não foi criado na VPC $vpc_id"
    exit 1
fi
...
--tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=gestaodepontos-dev}]' \
