...
security_group_id=$(aws ec2 describe-security-groups --group-names "gestaodepontos-dev" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)
if [ $? -eq 0 ]; then
  echo "[OK] Security Group gestaodepontos-dev foi criado"
  
  # Validar inbound rule para o security group 'gestaodepontos-dev'
  ...
  # Validar outbound rule para o security group 'gestaodepontos-dev'
  ...
  echo ">[ERRO] Não achei o security group gestaodepontos-dev. Ele foi criado?"
fi
